/* AE3301 ADMIN — author & manage questions from the Pad */
...
function merge(rows) {
  rows.forEach(r => {
    ...
    const q = { id: 'x-' + r.id, tier: r.tier, tags: [(r.topic || '').replace('cb-', '') || 'custom'], q: r.q, options: opts, answer: ans, explain: r.explain, hint: r.hint || 'Think it through.', wrongWhy: opts.map((o, i) => i === ans ? '' : 'Not quite — read the explanation after answering.') };
    (QUESTIONS[r.topic] = QUESTIONS[r.topic] || []).push(q);
  });
}
...
function form() {
  overlay('<h3 style="text-transform:uppercase">Author a question</h3>' +
    ...
    root => {
      root.querySelector('#a-save').onclick = async () => {
        ...
        const body = { topic: root.querySelector('#a-t').value, tier: root.querySelector('#a-tier').value, q: root.querySelector('#a-q').value, options: opts, explain: root.querySelector('#a-e').value, hint: root.querySelector('#a-h').value };
        ...
        if (d.ok) {
          ...
          root.querySelector('#a-msg').textContent = '✔ Published to every device on the network.';
          setTimeout(() => root.remove(), 900);
          window.dispatchEvent(new HashChangeEvent('hashchange'));
        } else root.querySelector('#a-msg').textContent = 'Server error — run python3 server.py';
      };
      root.querySelector('#a-man').onclick = async () => {
        ...
        root.querySelector('.modal-card').innerHTML = '<h3 style="text-transform:uppercase">Manage questions</h3>' +
          (rows.map(r => '<div class="plan-step"><span style="flex:1">' + r.q + ' <span class="mono faint">(' + r.tier + ')</span></span><button class="btn btn-danger btn-sm" data-del="' + r.id + '">DEL</button></div>').join('') || '<p class="muted">Nothing authored yet.</p>') +
          '<div class="cp-actions"><button class="btn btn-ghost" data-close2>CLOSE</button></div>';
        root.querySelectorAll('[data-del]').forEach(b => b.onclick = async () => {
          await fetch('/api/qdel', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: b.dataset.del }) });
          const i = QUESTIONS && Object.values(QUESTIONS).forEach(arr => { const k = arr.findIndex(x => x.id === 'x-' + b.dataset.del); if (k > -1) arr.splice(k, 1); });
